...
def receive_log():
    data = request.json
    message = data.get("log", "Không có nội dung log")
    save_log(message)
...
def chat_with_groq():
    ...
    # Lấy logs và dữ liệu mạng gần đây
    logs = fetch_recent_logs()
    network_data = fetch_recent_network_data()

    # Ghép ngữ cảnh tiếng Việt để hướng AI tốt hơn
    context_message = (
        f"{user_message}\n"
        f"Hệ thống hiện có CPU: {cpu}%, RAM: {memory}%, Ổ cứng: {disk}%.\n"
        f"5 log mới nhất: {logs}\n"
        f"Thông tin 5 IP gần đây: {network_data}\n"
        "→ Hãy đánh giá và trả lời ngắn gọn: có nên chặn IP không? Trả lời bằng tiếng Việt."
    )

    payload = {
        "model": "llama3-8b-8192",  # Bạn có thể thay đổi model tùy config
        "messages": [{"role": "user", "content": context_message}]
    }

    try:
        response = requests.post(GROQ_ENDPOINT, headers=GROQ_HEADERS, json=payload)
        print("🔁 Groq response raw:", response.text)

        if response.status_code != 200:
            assistant_message = f"❌ Groq API lỗi {response.status_code}: {response.text}"
        else:
            response_data = response.json()
            assistant_message = (
                response_data.get("choices", [{}])[0]
                .get("message", {}).get("content") or "❌ Groq không phản hồi gì."
            )

    except Exception as e:
        assistant_message = f"⚠️ Lỗi khi gọi Groq: {e}"

    # Lưu vào log và gửi trả
    save_log(f"Người dùng: {user_message}, AI: {assistant_message}")
