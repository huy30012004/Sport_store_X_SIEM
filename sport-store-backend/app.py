...
# 👉 Gửi log sang SIEM
    send_log_to_siem("👀 Truy cập danh sách sản phẩm từ website bán hàng.")
...
def create_order():
    ...
    save_data(data)
# 👉 Gửi log sang SIEM
    send_log_to_siem(f"🛒 Đơn hàng mới vừa được tạo, ID: {new_order['id']}")
...
def send_log_to_siem(message):
    try:
        ...
        print("⚠️ Không gửi được log đến SIEM:", e)

# Đếm số lượng request từ mỗi IP trong 10 giây gần nhất
ip_counter = defaultdict(list)

@app.before_request
def detect_ddos():
    # Ưu tiên lấy IP từ header nếu có (giả sử chạy sau proxy/nginx)
    ip = request.headers.get('X-Forwarded-For', request.remote_addr)
    print(f"📌 Request từ IP: {ip}")
    ...
    if len(ip_counter[ip]) > 30:
        send_log_to_siem(f"🚨 Phát hiện nghi ngờ DDoS từ IP {ip}: {len(ip_counter[ip])} yêu cầu trong 10 giây")
